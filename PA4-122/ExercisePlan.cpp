...
using std::endl;
using std::string;
//big three functions for class
ExerList::ExerList(ListNode* pNewHead, ListNode* pNewTail)
{
	this->mpHead = pNewHead;
	this->mpTail = pNewTail;
}

ExerList& ExerList::operator= (const ExerList& rhs) {
...
//puts node at end of the exercise list
bool ExerList::endList(ListNode* dayplan) {
	//replaces makeNode()
	bool success = false;
...
//allows editing of nodes within the exercise class
bool ExerList::editValue() {
	bool success = false;

	ListNode* pMem = this->mpHead;

	cout << "Which Exercise Goal number for this week would you like to edit" << endl;
	int choice = 0;
	int changedDat = 0;
	do {
		cin >> choice;
...
//getters and setters
ListNode* ExerList::getmpHead() const {
	return this->mpHead;
}

ListNode* ExerList::getmpTail() const {
	return this->mpTail;
}

void ExerList::setmpHead(ListNode* const pNewHead) {
	this->mpHead = pNewHead;
}

void ExerList::setmpTail(ListNode* const pNewTail) {
	this->mpTail = pNewTail;
}
